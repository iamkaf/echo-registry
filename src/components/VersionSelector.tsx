                ...
                <div className="absolute top-full left-0 right-0 mt-2 glass-panel max-h-[350px] flex flex-col shadow-xl z-50 overflow-hidden">
                    ...
                    <div className="overflow-y-auto flex-1 p-1">
                        {filteredVersions.length === 0 ? (
                            <div className="p-3 text-sm text-zinc-500 text-center font-mono">No versions found</div>
                        ) : (
                            filteredVersions.map((v) => (
                                ...
                                >
                                    ...
                                    {v.version_type !== 'release' && (
                                        <span className={cn("text-[10px] uppercase px-1.5 py-0.5 rounded ml-2",
                                            v.id.includes('-pre') || v.id.includes('-rc') ? "bg-amber-500/10 text-amber-500/70" : "bg-blue-500/10 text-blue-500/70"
                                        )}>
                                            {v.version_type}
                                        </span>
                                    )}
